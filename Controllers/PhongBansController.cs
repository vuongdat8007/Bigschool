...
namespace Bigschool_TH_11.Controllers
{
    public class PhongBansController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AssignCBNVsToPhongBan(string MaPhongBan, List<string> cbnvIds)
        {
            if (ModelState.IsValid)
            {
                var phongBan = db.PhongBans.Find(MaPhongBan);
                if (phongBan != null)
                {
                    foreach (var cbnvId in cbnvIds)
                    {
                        var existingCBNV = db.CBNVs.Find(cbnvId);
            ...
            return View("AssignCBNVsToPhongBan");
        }
